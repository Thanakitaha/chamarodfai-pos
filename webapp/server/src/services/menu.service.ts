...
export async function createMenuItem(payload: Omit<MenuItem,'id'|'createdAt'|'updatedAt'>) {
  ...
  `;
  const params = [STORE_ID, name, price, cost ?? 0, description ?? null, image ?? null, categoryId ?? null, available ?? true];
  const { rows } = await pool.query(sql, params);
  return mapMenuRowToApi(rows[0]);
}

export async function updateMenuItem(id: number, patch: Partial<MenuItem>) {
  const fields: string[] = [];
  const vals: any[] = [];
  let i = 1;

  const map: Record<string,string> = {
    name:'name', price:'price', cost:'cost', description:'description',
    image:'image_url', categoryId:'category_id', available:'available'
  };
  ...
  if (!fields.length) {
    const r = await pool.query(`SELECT menu_item_id AS id, name, price, cost, description, image_url AS image, available, category_id, created_at, updated_at FROM pos.menu_items WHERE menu_item_id=$1 AND store_id=$2`, [id, STORE_ID]);
    return r.rows[0] ? mapMenuRowToApi(r.rows[0]) : null;
  }
  const sql = `
    UPDATE pos.menu_items SET ${fields.join(', ')}, updated_at = now()
...
export async function deleteMenuItem(id:number) {
  const { rowCount } = await pool.query(
    `UPDATE pos.menu_items SET deleted_at = now()
