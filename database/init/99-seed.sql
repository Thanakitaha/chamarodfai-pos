...
SELECT
  ...
  (SELECT category_id
     ...
     ORDER BY category_id ASC LIMIT 1),
  v.name, v.price, v.cost, TRUE, 0
  ชาเย็นแก้ว ต้นทุน 15
ชาเย็นขวด ต้นทุน 15
ชาเขียวแก้ว ต้นทุน 15
ชาเย็นขวด ต้นทุน 15
ชาดำเย็นแก้ว ต้นทุน 15
ชาดำเย็นขวด ต้นทุน 15
ชาเงาะ ต้นทุน 35
ชาลิ้นจี่ ต้นทุน 35
โกโก้ ต้นทุน 15
FROM (SELECT store_id FROM pos.stores ORDER BY store_id ASC LIMIT 1) s,
     (VALUES
        ('ชาไทย (แก้ว)', 35.00, 15.00),
        ('ชาดำเย็น (แก้ว)', 35.00,  15.00),
        ('ชาเขียว (แก้ว)', 35.00,  15.00),
        ('โกโก้ (แก้ว)', 35.00,  15.00),
        ('ชาไทย (ขวด)', 35.00,  15.00),
        ('ชาดำเย็น (ขวด)', 35.00,  15.00),
        ('ชาเขียว (ขวด)', 40.00,  15.00),
        ('ชาลิ้นจี่ (แก้ว)', 65.00, 35.00)
     ) AS v(name, price, cost)
WHERE EXISTS (
  SELECT 1 FROM pos.menu_categories
  WHERE store_id = s.store_id AND name = 'ชา'
