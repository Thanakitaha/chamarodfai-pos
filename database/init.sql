-- =============================================================================
-- POS schema for database: chamarodfai   (PostgreSQL)
-- One-shot script for DBeaver (safe IF NOT EXISTS, ordered creation)
-- =============================================================================

-- ── Extensions (ต้องมีสิทธิ์) ───────────────────────────────────────────────
-- ถ้าไม่มีสิทธิ์ ให้ให้ superuser รัน 2 บรรทัดนี้ล่วงหน้า
...
CREATE TABLE IF NOT EXISTS pos.order_items (
  ...
  price           NUMERIC(12,2) NOT NULL CHECK (price >= 0), -- ราคาต่อหน่วย ณ เวลาขาย
  discount        NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (discount >= 0),
  tax_amount      NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (tax_amount >= 0),
...
CREATE TABLE IF NOT EXISTS pos.stock_movements (
  ...
  quantity        NUMERIC(14,3) NOT NULL,            -- + เพิ่ม / - ลด
  uom             TEXT NOT NULL DEFAULT 'unit',
  unit_cost       NUMERIC(12,4),
  total_cost      NUMERIC(14,2),
  source_table    TEXT,                              -- เช่น 'purchase_orders','orders'
  source_id       BIGINT,
  note            TEXT,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ── Cost lookup function (latest movement cost → fallback ingredient cost) ──
...
-- ── Consume stock by recipe & compute COGS on paid ──────────────────────────
...
BEGIN
  ...
  UPDATE pos.orders o
  SET cogs = COALESCE((
        SELECT ROUND(COALESCE(SUM(oi.cost_at_sale),0),2)
        FROM pos.order_items oi WHERE oi.order_id = p_order_id
      ),0),
      gross_profit = ROUND(o.total - COALESCE((
        SELECT COALESCE(SUM(oi.cost_at_sale),0)
        FROM pos.order_items oi WHERE oi.order_id = p_order_id
      ),0), 2)
...
-- ── Reverse stock when order voided/refunded ────────────────────────────────
...
-- ── Trigger: run auto consume/reverse on status change ───────────────────────
